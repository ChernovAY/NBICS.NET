...
class OutgoingRequestsViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "OutgoingCell", for: indexPath) as? OutgoingCell {
    ...
    private func Load(_ b:Bool=true) {
        if(b) {
            fArray = VSMAPI.Data.getContacts(type: VSMContact.ContactType.Out)
        } else {
            fArray.removeAll()
        }
